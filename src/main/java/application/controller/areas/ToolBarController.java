...
public class ToolBarController {
    ...
    public void onLeftClickPressed(int x, int y) {
        int shapeId = view.getShapeId(x, y);

        if (shapeId != -1)
            mainController.setHoldedShape(model.getShape(shapeId), model);
    }
    ...
    public void onLeftClickReleased(int x, int y) {
        if (mainController.isHoldingShape() && mainController.getHoldedShapeOrigin() != model) {
            ...
            mainController.setHoldedShape(model.getShape(view.getShapeId(x, y)), model);
        }
    }
}
